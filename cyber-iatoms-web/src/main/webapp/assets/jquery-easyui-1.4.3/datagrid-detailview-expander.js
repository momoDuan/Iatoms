var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		/**
		 * 根據要merge的字段來計算行號和rowspan等
		 */
		if(opts.mergeFileds && opts.mergeFileds.length != 0){
			var fristMergeFiled = opts.mergeFileds[0];
			var rows = state.data.rows;
			var nextMergeFiledName;
			var currentMergeFiledName;
			var indexRow = 0;
			var rowIndex = 0;
			for (var i = 0; i < rows.length; i ++) {
				currentMergeFiledName = rows[i][fristMergeFiled];
				for (var j = i + 1; j < rows.length; j ++) {
					indexRow ++;
					nextMergeFiledName = rows[j][fristMergeFiled];
					if (currentMergeFiledName != nextMergeFiledName) {
						rowIndex ++;
						for (var m = i; m < j; m ++) {						
							rows[m].dataSpan = indexRow;
							rows[m].rowIndex = rowIndex;
							if (m == i) {
								rows[m].isStartRow = true;
							} else {
								rows[m].isStartRow = false;
							}
						}
						i = j - 1;
						indexRow = 0;
				...
				if (i == rows.length - 1) {
					rowIndex ++;
					rows[i].rowIndex = rowIndex;
					rows[i].dataSpan = 1;
					rows[i].isStartRow = true;
				}
			}
		}
		...
		for(var i=0; i<rows.length; i++) {
			...
			if (i == rows.length - 1) {
				...
				if(opts.mergeFileds && opts.mergeFileds.length != 0){
					var fristMergeFiled = opts.mergeFileds[0];
					currentMergeFiledName = rows[i][fristMergeFiled];
					nextMergeFiledName = rows[i + 1][fristMergeFiled];
					if (nextMergeFiledName != currentMergeFiledName) {
						addDetail = true;
					}
				}
	...
	renderRow: function(target, fields, frozen, rowIndex, rowData){
		...
		if (frozen && opts.rownumbers){
			...
			if (opts.pagination){
				...
				rowData.rowIndex += (opts.pageNumber-1)*opts.pageSize;
			}
			/**
			 * 不是起始行時增加隱藏的td
			 */
			if (rowData.isStartRow) {
				//cc.push('<td field="' + field + '" ' + cls + ' ' + style + ' rowSpan="' + rowData.dataSpan + '"' + '>');
				cc.push('<td class="datagrid-td-rownumber" rowSpan="' + rowData.dataSpan + '"><div class="datagrid-cell-rownumber">'+rowData.rowIndex+'</div></td>');
			} else {
				style = 'style="display:none;"';
				cc.push('<td class="datagrid-td-rownumber"' +  style + '><div class="datagrid-cell-rownumber">'+rowData.rowIndex+'</div></td>');
			}
		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				 ...
				 */
				var flag = false;
				if(opts.allFileds && opts.allFileds.length != 0){
					//flag = arrayContainsStr(opts.allFileds, field);
					if($.inArray(field, opts.allFileds) != -1){
						flag = true;
					}
				}
				/**
				 * 用來處理其他需要合併的字段列
				 */
				if (field == "_expander" || flag) {
					if (rowData.isStartRow) {
						cc.push('<td field="' + field + '" ' + cls + ' ' + style + ' rowSpan="' + rowData.dataSpan + '"' + '>');
					} else {
						style = 'style="display:none;"';
						cc.push('<td field="' + field + '" ' + cls + ' ' + style + ' data-rowSpan="' + rowData.dataSpan + '"' + '>');
					}
				} else {
	...
	bindEvents: function(target){
		...
		body.unbind('click').bind('click', function(e){
			...
			if (tt.hasClass('datagrid-row-expander')){
				...
				var rowSpan = parseInt(tr.children().eq(1).attr('rowSpan'));
				var rowNewIndex = rowIndex + rowSpan -1;
				if (tt.hasClass('datagrid-row-expand')){
					$(target).datagrid('expandRow', rowNewIndex);
				} else {
					$(target).datagrid('collapseRow', rowNewIndex);
				}
	...
	onBeforeRender: function(target){
		 ...
		 * 得到需要meger的字段名
		 */
		var dataGridId = state.options.id;
		var mergeFileds = $("#"+dataGridId+"-"+"mergeFiled").val();
		state.options.mergeFileds = mergeFileds.split(",");
		var allFileds = $("#"+dataGridId+"-"+"fileds").val();
		state.options.allFileds = allFileds.split(",");
...
$.extend($.fn.datagrid.methods, {
	...
	getExpander: function(jq, index){	// get row expander object
		...
		if (td.is(":hidden")) {
			var rowspan = td.attr("data-rowSpan");
			index = index - parseInt(rowspan) + 1;
		}
