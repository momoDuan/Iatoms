...
(function(){
    ...
    qq.basePrivateApi = {
        ...
        _formatSize: function(bytes){
            ...
            var size = Math.max(bytes, 0.1).toFixed(1);
            if (size.split(".")[1] == '0') {
            	return Math.max(bytes, 0.1).toFixed(0) + this._options.text.sizeSymbols[i];
            }
        ...
        _maybeParseAndSendUploadError: function(id, name, response, xhr) {
            ...
            if (!response.success){
                ...
                } else if(xhr && xhr.status == 200 && !response.error){
