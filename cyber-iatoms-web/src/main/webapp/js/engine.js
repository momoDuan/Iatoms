...
var windowLocationPathname = ["admUser.do",
							  "changePassword.do",
							  "admRole.do",
							  "systemLog.do",
							  "baseParameterManager.do",
							  "contract.do",
							  "contract.do",
							  "merchantHeader.do",
							  "company.do",
							  ...
							  "assetTransfer.do",
							  "assetManage.do",
							  "dmmCheckTransInfo.do",
							  "assetStocktackHistory.do",
							  "assetStocktake.do",
							  "assetTransInfoHistory.do",
							  "assetStockReport.do",
							  "assetStatusReport.do",
							  "customerAssetList.do",
							  "caseHandle.do",
							  "dtidDef.do",
							  "edcParameter.do",
							  "workTemplate.do",
							  ...
							  "edcPaymentReport.do",
							  "oldDataTranfer.do",
							  "userColumnTemplate.do"];
var windowLocationSearch = "";
var windowNameReplace = "";
...
(function() {
  ...
  dwr.engine.defaultErrorHandler = function(message, ex) {
    ...
    else if (message.indexOf("0x80040111") != -1 || message.toUpperCase().indexOf("FAILED TO READ INPUT")>=0) dwr.engine._debug(message);
  };
  ...
  dwr.engine.remote = {
    ...
    handleNewWindowName:function(windowName) {
      ...
      if (windowNameReplace != null && windowNameReplace) {
        dwr.engine._debug("- Warning: This will override existing name of: " + windowNameReplace);
      }
  ...
  dwr.engine.transport = {
    ...
    xhr:{
       ...
       */
      processCometResponse:function(rponse, batch) {
        if (batch.charsProcessed == rponse.length) return;
        if (rponse.length == 0) {
          batch.charsProcessed = 0;
          return;
        }

        var firstStartTag = rponse.indexOf("//#DWR-START#", batch.charsProcessed);
        if (firstStartTag == -1) {
          // dwr.engine._debug("No start tag (search from " + batch.charsProcessed + "). skipping '" + response.substring(batch.charsProcessed) + "'");
          batch.charsProcessed = rponse.length;
          return;
        }
        // if (firstStartTag > 0) {
        ...
        var lastEndTag = rponse.lastIndexOf("//#DWR-END#");
        if (lastEndTag == -1) {
        ...
        if (rponse.charCodeAt(lastEndTag + 11) == 13 && rponse.charCodeAt(lastEndTag + 12) == 10) {
        ...
        var exec = rponse.substring(firstStartTag + 13, lastEndTag);
  ...
  dwr.engine.batch = {
    ...
    create:function() {
      var batch = {
        ...
        handlers:[],
        isPoll:false,
        map:{ callCount:0, windowName:windowNameReplace },
        paramCount:0,
        preHooks:[],
        postHooks:[],
        timeout:dwr.engine._timeout,
    ...
    createPoll:function() {
      var batch = {
        ...
        isPoll:true,
        map:{ windowName:windowNameReplace },
        paramCount:0,
        path:dwr.engine._pathToDwrServlet,
        preHooks:[],
        postHooks:[],
        timeout:0,
        windowName:windowNameReplace,
    ...
    prepareToSend:function(batch) {
      ...
      var pathName = "";
      if ($.inArray(pathName, windowLocationPathname) == -1) {
    	  pathName = '';
      }
      // security details are filled in late so previous batches have completed
      batch.map.page = encodeURIComponent(pathName + windowLocationSearch);
      batch.map.httpSessionId = dwr.engine._getHttpSessionId();
      batch.map.scriptSessionId = dwr.engine._scriptSessionId;
      batch.map.windowName = windowNameReplace;
