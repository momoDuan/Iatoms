...
function show_msg(errorMsg){
	msgarea.style.display="inline";
	msgarea.style.fontSize="16";
	msgarea.innerHTML=errorMsg;
	setTimeout("msgarea.innerHTML=''",360000);
	setTimeout("msgarea.style.display='none'",360000);
}

function checkDate(T1,tmDESCRIPT){
	startdate = trimString(T1.value);
	if(isNaN(Math.abs(startdate))){
		alert(tmDESCRIPT+"日期有誤");
		return false;
	}
