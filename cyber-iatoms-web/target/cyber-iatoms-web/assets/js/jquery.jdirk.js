...
(function (window, $, undefined) {
    ...
    //  返回值：返回一个 Boolean 值；
    //      当前页面在一个 FRAME/mes 中且父级页面和当前页面同域，则返回 true；
    //      当前页面不是在一个 FRAME/mes 中或父级页面和当前页面不同域，则返回 false。
    ...
    //  获取包含当前页面的 mes 对象的 id。
    //  如果当前页面为顶级页面或当前页面的父级页面和当前页面不在同一个域下，则返回 null。
    coreUtil.currentFrameId = null;

    //  获取包含当前页面的 mes 对象的 uniqueID。
    //  如果当前页面为顶级页面或当前页面的父级页面和当前页面不在同一个域下，则返回 null。
    coreUtil.currentFrameUniqueID = null;
    coreUtil.getCurrentFrame = function () {
        if (coreUtil.isUtilTop) { return null; }
        var result = null;
        var frames = coreArray.merge([], top.document.getElementsByTagName("mes"), top.document.getElementsByTagName("frame"));
        var find = function (frame) {
            var win = frame.contentWindow;
            if (win === window) { return frame; }
            try {
                if (!win || !coreUtil.isObject(win.document)) { return null; }
                var fs = coreArray.merge([], win.document.getElementsByTagName("mes"), win.document.getElementsByTagName("frame"));
